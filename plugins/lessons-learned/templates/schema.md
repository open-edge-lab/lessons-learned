...
knowledge base. It mirrors `.claude-plugin/lessons-learned.config.json` but with
prose explanations. Both Claude and humans should read this when ingesting,
querying, or linting.
...
qualify. When in doubt, skip — `/lesson-add` is always available for manual
override.
...
The `/lesson-lint` command flags pages with inconsistent supersede/status state.
